...
int execute_line(char *line){    
    ...
    char *args[ARGS_SIZE];
    // Y lo rellenaremos con los cachitos de la instrucción introducida
...
int parse_args(char **args, char *line){
    ...
    while( token && seguir == 1){
        ...
        // A quí recogemos algunas excepciones:
        if(iterador == 0){ //No hemos introducido nada
            args[0] = NULL;
            
        }else{
            iterador--;
    
            if(args[iterador][0] == '#'){ // Si hay un comentario lo eliminamos
                args[iterador] = NULL;
...
// Función para pasar un proceso de primer a segundo plano
int internal_bg(char **args){
