...
int  jobs_list_remove(int pos){
    ...
    //Antes de eliminarlo, imprimiermos sus datos
    if(n_pids == 2 || (n_pids -1 ) == pos){ //Solo hay un elemento
        // Si solo hay un trabajo, entonces simplemente limpiamos
        jobs_list[pos].pid = 0;
...
//Manejador para cuando un hijo muera y evitar procesos zombies
void reaper(int signum){
    ...
    while( (ended = waitpid( -1 , &estado , WNOHANG )) > 0){
        ...
        if( ended == jobs_list[0].pid ){ 
            ...
            if(WTERMSIG(estado)){
			    sprintf(mensaje,"[reaper()→ Proceso hijo %d (%s) en foreground finalizado por señal %d]\n", jobs_list[0].pid, jobs_list[0].cmd, WTERMSIG(estado));
		    } else {
			    sprintf(mensaje, "[reaper()→ Proceso hijo %d (%s) en foreground finalizado con exit code %d]\n", ended, jobs_list[0].cmd, WEXITSTATUS(estado));
		    }
        ...
        memset(jobs_list[0].cmd, '\0', sizeof(jobs_list[0].cmd)); //Limpiamos comando
            ...
            //Imprimimos los datos del proceso que acaba de terminar
            fprintf(stderr,"[reaper()→ Proceso hijo %d en background (%s) finalizado con exit code 0]\n",jobs_list[indiceJobs].pid,jobs_list[indiceJobs].cmd,WEXITSTATUS(estado));
...
void ctrlc(int signum){
	...
	if(jobs_list[0].pid > 0){
		...
		sprintf(mensaje,"[ctrlc()→ Señal %d no enviada por %d (%s) debido a que no hay proceso en foreground]\n",SIGTERM, getpid(),minishell);
...
void ctrlz(int signum){
	...
	if(jobs_list[0].pid > 0){
		...
		if(strcmp(jobs_list[0].cmd, minishell) != 0){
            ...
            sprintf(mensaje,"[ctrlz()→ Señal %d enviada a %d (%s) por %d (%s)]\n", SIGSTOP, jobs_list[0].pid, jobs_list[0].cmd, getpid(), minishell);
			...
			sprintf(mensaje,"[ctrlz()→ Señal %d no enviada debido a que el proceso en foreground es el shell]\n", SIGSTOP);
		}
	} else {
		sprintf(mensaje,"[ctrlz()→ Señal %d no enviada por %d debido a que no hay proceso en foreground]\n",SIGSTOP, getpid());
	}
    write(1, mensaje, strlen(mensaje)); 
